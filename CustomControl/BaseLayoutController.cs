...
namespace CustomLayout
{
    ...
    {
        int GetDataCount();
        void SetCellContent(BaseLayoutCell cell, int index);
    }

    [RequireComponent(typeof(RectTransform), typeof(ScrollRect))]
    [DisallowMultipleComponent]
    public abstract class BaseLayoutController : UIBehaviour
    {
        ...
        private LinkedList<BaseLayoutCell> cells = new LinkedList<BaseLayoutCell>();

        private IScrollerDelegate listener;
        public IScrollerDelegate Listener
        {
            set
            {
                listener = value;
        ...
        //{
        //    base.OnValidate();
        //    if (cellObject && !cellObject.GetComponent<BaseLayoutCell>())
        //    {
        //        cellObject = null;
        //    }
        //}
        ...
        protected void ReloadData(bool isReset = false)
        {
            ...
            float contentSize = 0;
            for (int i = 0; i < listener.GetDataCount(); i++)
            {
        ...
        {
            var cell = Instantiate(cellObject).GetComponent<BaseLayoutCell>();
        ...
        private void UpdateCell(BaseLayoutCell cell, int index)
        {
            cell.dataIndex = index;
            if (cell.dataIndex >= 0 && cell.dataIndex < listener.GetDataCount())
            {
                if (scrollDirection == Direction.Vertical)
                {
                ...
                listener.SetCellContent(cell, index);
